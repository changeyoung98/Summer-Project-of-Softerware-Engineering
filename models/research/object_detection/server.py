...
def do_find(sock, addr):
    ...
    os.system('python photo_query.py --path ' + filename)
    scan_csv = open('D://triplet-reid/data/query.csv', 'r')
    ...
    scan_csv.close()
    if count == 0:
        data = "No person in the query image"
...
def do_scan(sock, addr, num):
    scan_csv = open('D://triplet-reid/data/scan.csv', 'w')
    ...
    for i in range(num):
        ...
        data = str(i) + " ok"
        sock.sendall(data.encode(encoding="utf-8"))
    scan_csv = open('D://triplet-reid/data/scan.csv', 'r')
    scan_reader = csv.reader(scan_csv)
    count = 0
    for row in scan_reader:
        temp = row
